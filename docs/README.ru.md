...
- `adapt()` возвращает `list[dict[str, object]]`, а `iter_adapt()` — итератор
  по тем же строкам.
...
число строк может расти как `product(max(1, len(Li)))`. `adapt()` материализует
все строки в памяти, а `iter_adapt()` выдаёт их лениво.

Используйте `max_rows`, чтобы заранее прервать слишком большое разворачивание.
Ограничение работает и для eager-, и для lazy-API:

```python
rows = OrderAdapter.adapt(payload, max_rows=10_000)
```

Для больших результатов обрабатывайте строки лениво:
